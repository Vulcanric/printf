...
/**
 * printHex_lower - Prints unsigned integer in hexa-decimal, lowercased
 ...
 */
int printHex_lower(va_list ap, char __attribute__((unused))*buff, int flag,
		int width, int precision, int len_modif)
{
	...
	if (flag == '-')
	{
		num_D_printed += print_Hex(lowerhex, num);
		padd = width - num_D_printed;
		while (padd > 0)
		{
